		...
		<div class="content-area-inner" style="padding-top:40px;">
			...
			<div class="dev-main eclipse-main">
				<div class="eclipse-main-container">	
					...
					<p>The BIRT Report Engine API (RE API) allows you to integrate the run-time part of BIRT into your application. This may mean integrating into a stand alone Java application, deploying as part of a servlet or embedding it within an RCP application. The engine supports the following operations:
					<ul class="eclipse-list">
						<li>Discover the set of parameters defined for a report.</li><br />
						<li>Get the default values for parameters.</li><br />
						<li>Run a report to produce HTML/Paginated HTML, WORD, XLS, PS, ODT, ODS, ODP or PDF output.</li><br />
						<li>Fetch an image or chart for a report.</li><br />
						<li>Export Report data to CSV</li><br />
					...
					It is no longer necessary to set BIRT Home in the EngineConfig class. Just add the BIRT jars to your classpath/buildpath.</p>
					
					<h2 class="topic-section-header padding-top-small">Installing the Engine</h2>
					<p>The first step is to download, install, and configure the BIRT engine. The engine is more than just a simple JAR file; it also includes a set of extensions (plug-ins), and a set of data drivers.<br />
					<ul class="eclipse-list">
						<li>First download the Report Engine from the <a href="http://download.eclipse.org/birt/downloads">Eclipse download site.</a></li><br />
						<li>This file will be named birt-runtime-version.zip.</li><br />
						<li>Unzip this file into a directory, such as c:\birtruntime.</li><br />
						<li>When you set the Engine Home, which is explained later, use C:/birtruntime/birt-runtime-version/ReportEngine as the value.</li><br />
						<li>Add the jars in the ReportEngine/lib directory to your classpath/buildpath.</li>
					</ul>
					</p>
					
					<p>Note that if you are using the RE API within an RCP/Eclipse application these steps are not needed. Just add the RE API plugins to your application and call the needed functions.</p>
					<p>The BIRT Viewer sample is also bundled with the Report Engine download. It is located under the WebViewerExample directory. If you decide to use this example, see <a href="viewer-setup.php">Viewer Setup</a> for an explanation. The Viewer uses the RE API to generate reports within the context of a servlet. It provides additional features, such as printing, TOCs, Page controls, etc. Before using the RE API, verify that the Viewer is not applicable to your application.</p>
					
					<h2 class="topic-section-header padding-top-small">Engine Source</h2>
					...
					<h2 class="topic-section-header padding-top-small">Javadoc</h2>
					...
					<h2 class="topic-section-header padding-top-small">Configuring JDBC Drivers</h2>
					<p>You must configure the engine to include any JDBC drivers that you need.</p>
					
					<h3>Birt POJO Runtime Note: </h3>
					<p>If you are installing BIRT 3.7 or later See <a href="http://wiki.eclipse.org/Birt_3.7_Migration_Guide">BIRT 3.7 Migration Guide</a>.<br />
					It is no longer necessary to add the driver to the jdbc plugin. Add the driver jar to your classpath.</p>
					...
					<h2 class="topic-section-header padding-top-small">API Overview</h2>
					<p>The following are the key steps to use the API, identifying the classes and interfaces you use for each step.
					...
					<ul class="eclipse-list">
						<li>Set the Engine Home and start the Platform (Loads the plug-ins). If you are using the RE API in plugin format there is no need to set BIRT Home. If you are using the RE API from within a servlet, be sure to read about PlatformServletContext later on this page.</li></br>
						<li>Create an instance of the <code>ReportEngine</code> class. You can use this object to perform multiple tasks.</li><br />
						<li>Open a report design using one of the <code>openReport(&nbsp;)</code> methods of <code>ReportEngine</code>.</li><br />
						<li>Obtain information about report parameters using <code>IGetParameterDefinitionTask</code>. This is only needed if you want to build a custom parameter page or collect parameter definition information. The IRunTask and the IRunAndRenderTask interfaces have methods for setting parameter values.</li><br />
						<li>Run and render a report using <code>IRunAndRenderReportTask</code> or <code>IRunTask</code> and <code>IRenderTask</code>.</li><br />
						<li>Repeat steps 4-6 for the next report.</li><br />
					...
					<img src="/birt/test/img/documentation/integrating/retask.jpg" /><br /><br />
					
					<h2 class="topic-section-header padding-top-small">EngineConfig</h2>
					<h3>Birt POJO Runtime Note: </h3>
					<p>If you are installing BIRT 3.7 or later See <a href="http://wiki.eclipse.org/Birt_3.7_Migration_Guide">BIRT 3.7 Migration Guide</a>.<br />
					It is no longer necessary to set BIRT Home.</p>
					...
					<p>The engine home should be set to installedlocation/birt-runtime-version/ReportEngine when deployed as a stand alone Java Application. See the <a href="http://wiki.eclipse.org/index.php/BIRT_Report_Engine_API">wiki</a> examples for Servlet and RCP deployment.</p>
					<p>Other functions of interest within this class are:
					<ul class="eclipse-list">
						<li><code>
						setLogConfig(String directoryName, Level level)
						</code><br />
						This call sets the Log directory name and level (OFF, SEVERE, WARNING, INFO, CONFIG, FINE, FINER, FINEST and ALL)<br />
						Setting directoryName to null will log to stdout.</li>
					</ul>
					</p>
					
					<h2 class="topic-section-header padding-top-small">ReportEngine</h2>
					...
					<p>The report engine is created through a factory supplied by the Platform. Before creating the engine, you should start the Platform, which will load the appropriate plug-ins. This is done by calling <code>Platform.startup(config)</code> that takes an <code>EngineConfig</code> object as argument. After using the engine, call <code>Plaform.shutdown()</code> function to do clean up work, which includes unloading the extensions. When shutting down the engine in some environments, it may also be required to call:<br />
					<code>RegistryProviderFactory.releaseDefault();</code><br /><br />
					This should be added after the <code>Platform.shutdown()</code> method is called. You will need to import the <code>org.eclipse.core.internal.registry.RegistryProviderFactory</code> package. See <a href="https://bugs.eclipse.org/bugs/show_bug.cgi?id=351052">Bugzilla 351052</a> for more details.</p>
					<pre style="font-size: 10pt">
					try{
					...
					<ul class="eclipse-list">
						...
						Change engine log level (OFF, SEVERE, WARNING, INFO, CONFIG, FINE, FINER, FINEST and ALL)</li>
					</ul>
					</p>
					
					<h2 class="topic-section-header padding-top-small">IReportRunnable</h2>
					<p>BIRT report designs are stored as XML files.  By default the extension is rptdesign.<br />
					To work with the report design in the engine, you must first load the report using one of the <code>openDesign(&nbsp;)</code> methods in the <code>ReportEngine</code> class.</p>
					...
					<h2 class="topic-section-header padding-top-small">IEngineTask</h2>
					...
					<ul class="eclipse-list">
						<li><code>engine.createDataExtractionTask();</code></li><br />
						<li><code>engine.createGetParameterDefinitionTask();</code></li><br />
						<li><code>engine.createRenderTask();</code></li><br />
						<li><code>engine.createRunTask();</code></li><br />
						<li><code>engine.createRunAndRenderTask();</code></li>
					</ul>
					
					<h2 class="topic-section-header padding-top-small">IDataExtractionTask</h2>
					<p>Use this task to extract data from a report document.  The BIRT viewer uses this class to extract report data into CSV format. This class supports extracting data from the report document by specifying the result set and columns you would like to have extracted.<br />
					From the Viewer code<br />
					<pre style="font-size: 10pt">
					...
					<h2 class="topic-section-header padding-top-small">IGetParameterDefinitionTask</h2>
					...
					<p>The following example opens a report design and iterates through the parameters and parameter groups. If a List Box parameter is found, which is not in a group, the selection values are retrieved.<br /><br />
					<pre style="font-size: 10pt">
					...
					<h2 class="topic-section-header padding-top-small">IRenderTask</h2>
					...
					<p>The following example renders the first two pages of the "Pages" report document. You will notice that it renders the two pages as one page of HTML.
					<pre style="font-size: 10pt">
					...
					<h2 class="topic-section-header padding-top-small">IRunTask</h2>
					...
					<p>The following example simply creates a report document and saves it to disk.
					<pre style="font-size: 10pt">
					//Open a report design 
					...
					<h2 class="topic-section-header padding-top-small">IRunAndRenderTask</h2>
					...
					<p>Reports may take parameters. The <code>IRunAndRenderReportTask</code> takes parameter values as a <code>HashMap</code> or you can set them individually. The <code>IRunAndRenderReportTask</code> provides the <code>validateParameters(&nbsp;)</code> method to validate the parameter values before you run the report.
					<pre style="font-size: 10pt">
					...
					<h2 class="topic-section-header padding-top-small">Miscellaneous</h2>
					...
					<p>Add the following code to your task.
					<pre style="font-size: 10pt">
					...
					<p>Now within the the script editor, you can reference your Java Object as follows:
					<pre style="font-size: 10pt">
					...
					<p>When rendering a report it may be desirable to output the report to an output stream, such as <code>HttpServletResponse</code>. To do this modify your RenderOption instance to use <code>setOutputStream</code> instead of <code>setOutputFileName</code>. For example,
					<pre style="font-size: 10pt">
					...
					<p>The IPlatformContext interface describes the methods needed to load the resources required by the BIRT runtime.  Within BIRT there are two implementations of this interface, PlatformFileContext() and PlatformServletContext(). The Platform Context is set using the setEngineContext method of the <code>EngineConfig</code> class. If this method is not called it defaults to PlaformFileContext() and uses the Java File API to load the resources. The <code>PlatformServletContext</code> class uses Resource based operations. So if you are deploying an application to the Web that uses the BIRT API and it is not contained in a war you can use the default and set your engine home to something similar to:
					<pre style="font-size: 10pt">
					config.setEngineHome( servletContext.getRealPath("/WEB-INF"));
					</pre>
					</p>
					
					<p>The BIRT Viewer application will load the plug-ins from /WEB-INF/Plugins directory. If you deploy your application in a war, setup your code like:
					<pre style="font-size: 10pt">
					...
					<p>If you need to embed your report output into an existing web page you can use the <code>options.setEmbeddable(true);</code> method. This will remove the &lt;HTML&gt; and &lt;BODY&gt; tags in the generated report. For example,
					<pre style="font-size: 10pt">
					...
					<h2 class="topic-section-header padding-top-small">Emitter Configuration</h2>
					<p>An &quot;emitter&quot; is the component of the report engine that produces output. BIRT provides many emitters such as HTML, XLS and PDF. To configure emitter options you need to use the RenderOption class. In some of the examples above this was done, with code similar to the following:
					<pre style="font-size: 10pt">
					IRenderOption options = new RenderOption();		
					options.setOutputFormat("html");
					options.setOutputFileName("output/resample/eventorder.html");
					...
					<p>BIRT also provides an extended RenderOption class for the PDF and HTML emitters (PDFRenderOption and HTMLRenderOption). The PDFRenderOption class provides methods for handling fonts and the HTMLRenderOption class provides methods for handling images, url encoding, and other html specific settings. You may need to configure the HTML emitter to manage images.<br />
					BIRT supports several image types:
					<ul class="eclipse-list">
						<li>Images referenced using a URL</li><br />
						<li>Embedded images</li><br />
						<li>Images created to represent charts</li><br />
						<li>Images from BLOB types</li>
					</ul>
					</p>
					
					<p>Your application must have a policy for handling images in HTML. URL-based images are usually no problem. However, you'll need to handle the others. The <code>IHTMLImageHandler</code> interface defines the policy. The BIRT Engine provides two default implementations:
					<ul class="eclipse-list">
						<li><code>HTMLCompleteImageHandler</code>: used to write images to disk when rendering a report to produce an HTML file on disk.</li><br />
						<li><code>HTMLServerImageHandler</code>: used to handle images for an engine running in an app server. This class is used by the BIRT example web viewer.</li><br />
					</ul>
					</p>
					...
					<p>Setting the Image Directory instructs the RE API to store images, used within the report output, in that directory. If your emitter is setup with the standard <code>HTMLCompleteImageHandler</code>, the output image src attribute will point to this directory using the File protocol. If your emitter is configured with the <code>HTMLServerImageHandler</code>, you can specify an image base URL using the <code>HTMLRenderOption</code> class. The engine will prepend all images with this URL in the src attribute of the report output.
					<pre style="font-size: 10pt">
					HTMLRenderOption options = new HTMLRenderOption();		
					options.setOutputFileName("output/resample/TopNPercent.html");
					options.setOutputFormat("html");
					...
					<ul class="eclipse-list">
						<li><a href="http://www.birt-exchange.org/org/devshare/deploying-birt-reports/986-webinar-archive-using-the-birt-report-engine-apis/">Report Engine API Webinar</a></li><br />
						<li><a href="http://www.birt-exchange.org/org/devshare/deploying-birt-reports/1344-eclipsecon-2011-api-examples/">Report Engine API Examples</a></li><br />
						<li><a href="http://www.birt-exchange.org/org/devshare/?topicid=0&filtercontent=engine+api&contenttype=&timeperiod=0&results=10">Report Engine API Examples in the BIRT Developer Center DevShare</a></li>
					</ul>
					</p>
